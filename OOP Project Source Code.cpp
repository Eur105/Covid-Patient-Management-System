#include <iostream>
#include <cstring>
#include <windows.h> //contains declaration for all the functions in the window API
#include <fstream>
...
class AdminMessage : public GreetingMessage {
	public:
		void printmessage() {
...
class UserMessage : public GreetingMessage {
	public:
		void printmessage() {
...
void Main_control::menu() {
	...
	switch (choice) {
		...
		case 3:
			system("cls");
			cout << "\n\n\t\t\t COVID-19 MANAGEMENT SYSTEM - BY SAFWAN MUNEEB AND EBAD";
			Sleep(10); // suspendds the execution of the program for 10 secs
			exit(0); //terminates the program
		default:
			cout << "\n\n\t\t\t Enter a valid choice... Please Try Again....";
			cout << "\n\n Press Any Key To Continue: ";
			getch(); //used to read a aisngle char
			menu();
	}
}
...
void Main_control::valid(string str) { // Check Username is available or not
	...
	dir = str + ".txt";
	file.open(dir.c_str());
	if (!file.is_open() && file.fail()) { //file is open and ready to be used
		return;          //tells that previous operation failed               
	} else {
		tm++;
		if (tm == 3) {
		...
		cout << "\n\n\t\tCreate a Username: ";
		cin >> usn;
		valid(usn); // returns whether future object is currently associated to any file
	}
}
...
void Main_control::user_password() {
	...
	if (user_choice == 1) {
		...
		cout << "\n\n\t\t -->> REGISTRATION USER <<--";
		cout << "\n\n\t\tEnter Your Name: ";
		cin.ignore(); //clear a char from input buffer
...
void Main_control::display_doctor_data() {
	...
	cout << "\n\n\t\t-->> SHOW DOCTOR DATA <<--";
	ifstream file;
	file.open("Doctor_Data.txt", ios::in | ios::binary);//allows read operation from file
	if (!file) {
		cout << "\nFile Not Found!";
	} else {
...
B:
	...
	getline(cin,address);
	cout << "\n\n\t\tEnter Vaccine Name [Pfizer, extrazenica, moderna, cinovac or cansino]: ";
